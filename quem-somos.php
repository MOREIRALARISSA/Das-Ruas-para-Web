<?php
    include ('_templates/header.php');
    //Incluido o menu de navegação
?>

<main class="my-2">
    <div class="container">
        <article role="article">
            <section>
                <h1 class="text-center mb-4" role="heading">Quem Somos</h1>
                <div class="row justify-content-lg-center mb-5">
                    <div class="col-lg-4 col-md-6 col-sm-12">
                        <div class="card mt-2 mb-3 cardb">
                            <div class="card-fundo w-100"><img class="img-fluid" src="_images/quem-somos/fundo-larissa.jpg" alt="Adelina"></div>
                            <div class="avatar mx-auto">
                                <img src="_images/quem-somos/adelina.jpg" alt="Adelina">
                            </div>
                            <div class="card-body">
                                <h4 class="card-title font-weight-bold" role="heading">Adelina Ferreira</h4>
                                <small>Coder</small>
                                <p>RA: <strong>2650831913016</strong></p>
                                <hr>
                                <p>Adelina Ferreira é estudante do curso sistemas para intenet na fatec de São Roque, ela ama arte conteporanea </p>
                                <h4>Áreas</h4>
                                <div><a href="_authors/adelina/index.php">3D</a></div>
                                <!--                <div><a href="#">Pages centers</a></div>-->
                            </div>
                        </div>
                    </div>

                    <div class="col-lg-4 col-md-6 col-sm-12">
                        <div class="card mt-2 mb-3 cardb">
                            <div class="card-fundo"><img class="img-fluid" src="_images/quem-somos/fundo-larissa.jpg" alt="Adelina"></div>
                            <div class="avatar mx-auto">
                                <img src="_images/quem-somos/anderson.jpg" alt="Anderson">
                            </div>
                            <div class="card-body">
                                <h4 class="card-title font-weight-bold" role="heading">Anderson Antunes</h4>
                                <small>Manager | Coder</small>
                                <p>RA: <strong>2650831913016</strong></p>
                                <hr>
                                <p>Anderson Antunes, atleta de fim de semana, aspirante em tecnologia / programação.</p>
                                <h4>Áreas</h4>
                                <div><a href="_authors/anderson/index.php">Throwup</a></div>
                                <div><a>Pages centers</a></div>
                            </div>
                        </div>
                    </div>

                    <div class="col-lg-4 col-md-6 col-sm-12">
                        <div class="card mt-2 mb-3 cardb">
                            <div class="card-fundo"><img class="img-fluid" src="_images/quem-somos/fundo-larissa.jpg" alt="Adelina"></div>
                            <div class="avatar mx-auto">
                                <img src="_images/quem-somos/igor.jpg" alt="Igor">
                            </div>
                            <div class="card-body">
                                <h4 class="card-title font-weight-bold" role="heading">Igor Takeyama</h4>
                                <small>Coder</small>
                                <p>RA: <strong>2650831913016</strong></p>
                                <hr>
                                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eos, adipisci</p>
                                <h4>Áreas</h4>
                                <div><a href="_authors/igor/index.php">Freestyle</a></div>
                                <!--                <div><a href="#">Pages centers</a></div>-->
                            </div>
                        </div>
                    </div>
                </div>

                <div class="row justify-content-lg-center mb-5">
                    <div class="col-lg-4 col-md-6 col-sm-12">
                        <div class="card mt-2 mb-3 cardb">
                            <div class="card-fundo"><img class="img-fluid" src="_images/quem-somos/fundo-larissa.jpg" alt="Adelina"></div>
                            <div class="avatar mx-auto">
                                <img src="_images/quem-somos/larissa.jpg" alt="Larissa">
                            </div>
                            <div class="card-body">
                                <h4 class="card-title font-weight-bold" role="heading">Larissa Moreira</h4>
                                <small>Designer | Coder</small>

                                <p>RA: <strong>2650831913016</strong></p>
                                <hr>
                                <p>Belorizontina apaixonada por artes, design, tecnologia e é estudante de Sistemas para Internet na Fatec São Roque. </p>
                                <h4>Áreas</h4>
                                <div><a href="_authors/larissa/index.php">Mural</a></div>
                                <div><a>Pages centers</a></div>
                            </div>
                        </div>
                    </div>

                    <div class="col-lg-4 col-md-6 col-sm-12">
                        <div class="card mt-2 mb-3 cardb">
                            <div class="card-fundo"><img class="img-fluid" src="_images/quem-somos/fundo-larissa.jpg" alt="Adelina"></div>
                            <div class="avatar mx-auto">
                                <!--    <img src="_images/quem-somos/matheus.jpg" alt="Matheus"> -->
                            </div>
                            <div class="card-body">
                                <h4 class="card-title font-weight-bold" role="heading">Vitor</h4>
                                <small>Lorem ipsum </small>
                                <p>RA: <strong>Lorem ipsum</strong></p>
                                <hr>
                                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eos, adipisci</p>
                                <h4>Áreas</h4>
                                <div><a href="_authors/matheus/index.php">Stencil</a></div>
                                <div><a>Lorem ipsum</a></div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </article>
    </div>
</main>

<?php
    include ('_templates/footer.php');
    //Incluindo o rodapé
?>
